...
export class ProfilePerniagaanPage implements OnInit {
  ...
  constructor(
    ...
    private klusterService: KlusterService // private geolocation: Geolocation
  ) {
  ...
  async getPerniagaan() {
    ...
    this.perniagaanService.show(this.usahawan_id).subscribe((res) => {
      ...
      if (Object.keys(res).length === 0) {
        ...
        if (this.perniagaan.jenisperniagaan) {
          this.getNegeri();
          loading.dismiss();
          this.setFormValues();
        }
        ...
        this.produkService.get(res.id).subscribe((produkRes) => {
          ...
          this.negeriService.get().subscribe((resNegeri) => {
            console.log("Negeri in perniagaan", resNegeri);
            ...
            this.daerahService
              ...
              .pipe(
                map((x) =>
                  x.filter((i) => i.U_Negeri_ID == this.perniagaan.U_Negeri_ID)
                )
              )
              .subscribe((resDaerah) => {
                ...
                this.mukimService
                  ...
                  .pipe(
                    map((x) =>
                      x.filter(
                        (i) => i.U_Daerah_ID == this.perniagaan.U_Daerah_ID
                      )
                    )
                  )
                  .subscribe((resMukim) => {
                    ...
                    this.parlimenService
                      ...
                      .pipe(
                        map((x) =>
                          x.filter(
                            (i) => i.U_Negeri_ID == this.perniagaan.U_Negeri_ID
                          )
                        )
                      )
                            ...
                            this.kampungService
                              ...
                              .pipe(
                                map((x) =>
                                  x.filter(
                                    (i) =>
                                      i.U_Mukim_ID == this.perniagaan.U_Mukim_ID
                                  )
                                )
                              )
                                      ...
                                      .pipe(
                                        map((x) =>
                                          x.filter(
                                            (i) =>
                                              i.jenis_kluster ==
                                              this.perniagaan.jenisperniagaan
                                          )
                                        )
                                      )
                                      .subscribe((resKluster) => {
  ...
  getKluster() {
    ...
    this.klusterService
      ...
      .pipe(
        map((x) =>
          x.filter((i) => i.jenis_kluster == this.form.value.jenisperniagaan)
        )
      )
      .subscribe((res) => {
              ...
              .subscribe((res) => {
                ...
                for (let i = 0; i < prodTempLength; i++) {
                  ...
                  if (prodTemp[i].id == "") {
                    ...
                    this.produkService
                      .update(prodTemp[i], prodTemp[i].id)
                      .subscribe((prodRes) => {
                        console.log("prodRes", prodRes);
                      });
                  }
  ...
  calcMaklumatPendapatan() {
    ...
    this.aliranService.getTotalYear(this.user_id).subscribe((res) => {
      ...
      let peratus_kenaikan =
        ((jualan_tahunan_semasa - purata_tahunan_sebelum_bantuan) /
          purata_tahunan_sebelum_bantuan) *
        100;
      this.form.patchValue({
